...
class ExperimentHandler:
    ...
    def _compute_fwer(self, reject, alt_mask):
        """
        :params reject: матрица вердиктов размера (n_runs, 3)
        :params alt_mask: маска столбцов, которые в текущей
        конфигурации соответствуют справедливой альтернативной гипотезе
        """

        assert reject.shape == (self.n_runs, 3)

        # Если всё из альтернативы, то что FWER, что FDR равны нулю. 
        if np.all(alt_mask):
            return 0

        # FWER = P(V > 0) в нотации из _compute_fdr
        false_positives = reject[:, ~alt_mask]
        fwer = np.mean(np.any(false_positives, axis=1), axis=0)

        assert np.isscalar(fwer)
        return fwer

    def _compute_fdr(self, reject, alt_mask):
        """
        :params reject: матрица вердиктов размера (n_runs, 3)
        :params alt_mask: маска столбцов, которые в текущей
        конфигурации соответствуют справедливой альтернативной гипотезе
        ...
        # Если распределение на самом деле полностью
        # согласуется с нулевой гипотезой, то FDR
        # переходит в E I {V > 0} = FWER
        if ~np.any(alt_mask):
            return self._compute_fwer(reject, alt_mask)

        # Eсли все из альтернативы, то FDR равно нулю.
        if np.all(alt_mask):
            return 0

        # V — "false positives", R — "positives",
        # т.е. отвергнутые гипотезы
        V = reject[:, ~alt_mask].sum(axis=1)
        R = reject.sum(axis=1)
    ...
    def _compute_power(self, reject, alt_mask):
        """
        :params reject: матрица вердиктов размера (n_runs, 3)
        :params alt_mask: маска столбцов, которые в текущей
        конфигурации соответствуют справедливой альтернативной гипотезе
        ...
        # Если данные из основной гипотезы, то мы гарантированно
        # отвергаем всех, кто из альтернативы. Тем не менее,
        # в таком случае не совсем корректно говорить о мощности.
        if ~np.any(alt_mask):
            return np.nan

        alt_reject = reject[:, alt_mask]
        power = alt_reject.mean()
    ...
    def run(self):
        ...
        scatter_fig = plt.figure(figsize=(20, 9))
